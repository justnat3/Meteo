...
export class MapdashComponent implements OnInit {
  ...
  async weatherInfo() {
    ...
    (await this.WeatherAPI.getWeather()).subscribe((weather) => {
      this.res = weather;
      // reduce res.alert list to weatherDescription to display description value instead of description list. Mapping this value to a reduced array does not work
